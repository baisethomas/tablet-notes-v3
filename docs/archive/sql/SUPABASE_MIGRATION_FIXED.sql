-- TabletNotes Supabase Migration Script (FIXED)
-- Safe migration that preserves existing data
-- Handles existing table name: transcriptions (not transcripts)

-- ============================================================================
-- STEP 1: Rename transcriptions to transcripts (for consistency)
-- ============================================================================
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'transcriptions')
       AND NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'transcripts') THEN
        ...
        RAISE NOTICE 'Renamed transcriptions table to transcripts';
    END IF;
END $$;
...
BEGIN
    ...
    -- Add audio_file_name if missing
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='sermons' AND column_name='audio_file_name') THEN
        ALTER TABLE sermons ADD COLUMN audio_file_name TEXT;
        -- Try to extract filename from audio_file_url if it exists
        UPDATE sermons SET audio_file_name =
            CASE
                WHEN audio_file_url IS NOT NULL THEN split_part(audio_file_url, '/', -1)
                ELSE 'unknown.m4a'
            END
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
        ...
        -- Backfill with date column for existing records
        UPDATE sermons SET created_at = date WHERE created_at IS NULL;
        RAISE NOTICE 'Added created_at to sermons';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
        ...
        UPDATE sermons SET updated_at = COALESCE(created_at, NOW()) WHERE updated_at IS NULL;
        RAISE NOTICE 'Added updated_at to sermons';
    END IF;
END $$;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
        ...
        RAISE NOTICE 'Added created_at to transcripts';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
        ...
        RAISE NOTICE 'Added updated_at to transcripts';
    END IF;
END $$;
...
BEGIN
    ...
    -- Add title column if missing (for new AI-generated titles)
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
        ...
        RAISE NOTICE 'Added created_at to summaries';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
        ...
        RAISE NOTICE 'Added updated_at to summaries';
    END IF;
END $$;
