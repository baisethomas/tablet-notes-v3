...
exports.handler = withLogging('summarize-refresh', async (event, context) => {
  ...
  // Apply summary refresh rate limiting
  const rateLimitMiddleware = createRateLimitMiddleware('summary_refresh');
  const rateLimitResponse = await rateLimitMiddleware(event, context);
  if (rateLimitResponse) {
    logger.rateLimit(event.user?.id || 'anonymous', 'summary_refresh', false, {
  ...
  try {
    ...
    logger.info('User authenticated successfully for summary refresh', { userId: user.id });
    ...
    logger.info('Received text for summary refresh', {
      userId: user.id,
      textLength: text ? text.length : 0,
      textPreview: text ? text.substring(0, 200) + '...' : 'null',
      actualServiceType,
      userTier: user.user_metadata?.subscription_tier || 'free'
    });
    ...
    if (sanitizedText.length < 50) {
      logger.warn('Text too short after sanitization for refresh', {
        userId: user.id,
    ...
    logger.info('Processing summary refresh request', { 
      userId: user.id,
    ...
    const completionWithTimeout = withTimeout(
      () => openAIBreaker.execute(() => openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `# Tiered Sermon Summary System Prompt (REFRESH REQUEST)
...
IMPORTANT: This is a REFRESH request. Generate a fresh perspective on this content while maintaining the same high standards of accuracy and faithfulness. Vary your approach slightly to provide new insights or organize the content differently while staying true to the original message.
...
- **Fresh Perspective**: Since this is a refresh request, organize or present the content with a slightly different approach while maintaining accuracy
...
**Important**: If there is insufficient material for any premium section, omit that section entirely rather than adding content not present in the transcript. It's better to provide fewer sections with accurate content than to fill sections with invented material.`
          ...
          {
            role: "user",
            content: `Please provide a refreshed summary of this ${actualServiceType} text: ${sanitizedText}`
          }
        ],
    ...
    logger.error('Summary refresh request failed', {
