...
- **A column the client wrote stays client-supplied forever.** `sermons.audio_file_path` is persisted verbatim from `create-sermon`, and three later endpoints read it back as if the server had authored it — `jobs.js` even documented the assumption in a comment ("which it wrote itself and so does not need to re-authorize"). It then signed that path with the **service role key**, which bypasses RLS: a crafted row disclosed another user's audio, and `delete-sermon` destroyed it (TAB-82/84). Owning the row never implies owning the object it names. Authorize on **both** sides — refuse to store a foreign path, *and* re-check it on every read, since rows written before the write-side check still exist. One shared predicate (`utils/audioObjectPath.js#isOwnedObjectPath`), never an inline `startsWith`.
- **Path checks reject, they don't repair.** `{owner}/../{victim}/x.m4a` passes a prefix test; `%2e%2e` passes a literal `..` test; how many times a storage layer decodes is not a thing to reason about per call site. Refuse anything outside the shape we generate (`{uuid}/{uuid}.{ext}` — so no `%`, no `..`, no `\`) rather than sanitizing input into something that merely looks safe. Exception: legacy bare filenames at the bucket root (no `/`) are still accepted by `isOwnedObjectPath` — a few exist in prod and cannot name another user's namespaced object; do not "tighten" that away without a migration. Verify against prod before tightening (0 of 425 rows contained `%`/`..`/`\`), and pin both the legitimate namespaced shape and the bare-filename exception with tests so hardening can't quietly start rejecting real uploads.
...
| 6 | **The trusted client** — persisting tier/identity/paths the client claimed, then *reading them back* as server-authored (three endpoints signed a client-supplied storage path with the service role key) | Server derives from verified payloads only; authorize client-supplied paths on write **and** on every read (§6) |
...
- [ ] `npm test` green (all suites, currently 235) and `node --check` on every changed function
...
- [ ] **Any helper that gates access has adversarial tests, not just happy-path ones.** `checkResourceOwnership` guarded service-role (RLS-bypassing) storage access with *zero* tests and accepted traversal for months. A predicate deciding "may this caller touch this object?" is tested with the attack — foreign prefix, traversal, encoded traversal, sibling prefix (`{id}-evil/`), non-strings — *and* with the legitimate shape, so hardening can't silently start rejecting real traffic
