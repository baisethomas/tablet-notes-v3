...
class SupabaseService: SupabaseServiceProtocol {
    ...
    func getSignedUploadURL(for fileName: String, contentType: String, fileSize: Int, sermonLocalId: UUID? = nil) async throws -> (uploadUrl: URL, path: String) {
        ...
        // Opts this upload into a stable, retry-safe object path (TAB-73). The
        // server falls back to a random path when it is absent, so older builds
        // keep working.
        if let sermonLocalId {
            body["sermonLocalId"] = sermonLocalId.uuidString.lowercased()
        }
    ...
    /// Uploads the audio file, streaming it from disk (TAB-73).
    ///
    /// This previously did `Data(contentsOf: localUrl)`, holding the entire
    /// recording in memory: a 2-hour sermon is ~100MB, and prod already has a
    /// 33MB object. `upload(for:fromFile:)` hands URLSession the file so it
    ...
    func uploadAudioFile(at localUrl: URL, to signedUploadUrl: URL) async throws {
        try await NetworkRetry.withExponentialBackoff(maxAttempts: 3) {
            ...
            // Was 5 minutes, which a long sermon on church WiFi can exceed —
            // the transfer would be cancelled mid-flight and retried from zero.
